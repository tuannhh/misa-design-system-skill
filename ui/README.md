...
## Danh sách control (30)

| Control | File | Ghi chú nhanh |
|---|---|---|
| Icon | `MIcon.vue` + `iconRegistry.generated.js` | registry sinh từ bundle SVG đã xác minh · size 12–48 · filled chỉ khi có asset chính thức |
...
| Header bar | `MHeaderBar.vue` | cao 48px `light`/`brand` · app switcher 9 chấm · search secondary trước focus · Setting → AVA → Chat → Notification → Help → More → avatar |
| Sidebar | `MSidebar.vue` | trắng · expanded 240px / collapsed 56px hover popover · active rounded brand-50 trong gutter |
| Chart | `MChart.vue` | ECharts wrapper (cần `npm i echarts`) · palette chart tokens · số VN · auto-resize |
| Image viewer | `MImageViewer.vue` | fullscreen slideshow · Esc/←/→ · counter · thumbnail strip · responsive mobile |
| Collapse/Expand panel | `MCollapseExpandPanel.vue` | tab 40×16/16×40px · `side` top/bottom/left/right · icon chevron đảo chiều · dùng thu/mở KPI row |
| Switch | `MSwitch.vue` | toggle 36×20px · bật = nền success |
| Tree | `MTree.vue` | cây phân cấp làm phẳng (flatten) · single-select · checkbox cascade cha/con + indeterminate · node lá không chevron nhưng vẫn thẳng cột |
| Upload | `MUpload.vue` | dropzone kéo-thả + click · validate dung lượng (`maxSizeMB`) · KHÔNG tự upload — emit `select-files` cho cha xử lý |

**Helper không phải control**: `useFormValidation.js` — composable validate form dùng chung (`rules.required/email/phoneVN/minLength/maxLength/min/max` + `validate()` tự focus vào ô lỗi đầu tiên, khớp quy tắc "không disable nút Submit" của MDS). Field cần tự focus phải bọc `data-field="tênTrường"`. Xem ví dụ thật trong `ui/templates/FormPage.vue`.
...
- Icon luôn render qua `MIcon`; bundle thiếu thì đối chiếu Figma trước, bổ sung SVG chính thức vào MDS và chạy generator trước khi ứng dụng sử dụng.
