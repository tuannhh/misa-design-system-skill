...
**Icon AMIS Chat dùng đúng icon `message` trong thư viện** (`assets/icons/message.svg` qua `MIcon`, KHÔNG tự vẽ SVG riêng, KHÔNG dựng badge tròn màu cố định kiểu AVA — đã thử và SAI, giám đốc thiết kế phản hồi 2026-07-16 đợt 3). Tint màu theo `buttonClass` như icon thường: trắng trên header brand, `#1570EF` trên header sáng (KHÔNG chuyển `--icon-neutral`).
