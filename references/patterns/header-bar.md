...
| **Icon AMIS Chat** | **giữ nguyên badge tròn xanh `#1570EF`** (KHÔNG chuyển trắng/neutral) | **giữ nguyên badge tròn xanh `#1570EF`** |
...
**Icon AMIS Chat là asset màu CỐ ĐỊNH** (giống cách xử lý AVA): badge tròn nền xanh nhận diện `#1570EF` + bong bóng chat trắng bên trong, giữ nguyên ở mọi chế độ header — KHÔNG tint theo `buttonClass` trắng/neutral như icon thường (đối chiếu ảnh chụp sản phẩm thật: icon luôn hiển thị dạng badge tròn màu xanh). Sửa 2026-07-16 (đợt 2): bản trước từng để icon đổi trắng trên header brand — SAI, đã chốt lại theo ảnh tham chiếu.
