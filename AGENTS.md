...
2. Đọc file quy chuẩn tương ứng trong thư mục `references/` (cùng thư mục với file này):
   ...
   - `references/conventions.md` — phím tắt, chính tả, định dạng số VN, tiêu đề trình duyệt, icon dùng chung, chat AI, bản quyền.
...
- **Icon: Figma MDS là nguồn chuẩn; `assets/icons/` là gói lõi đã xác minh.** Tra bundle → bảng map → Figma; dùng `MIcon.vue`, không inline SVG/map path riêng. Nếu Figma có nhưng bundle thiếu, bổ sung đúng SVG rồi chạy generator. Chỉ tìm Tabler/tự vẽ sau khi xác minh Figma chưa có.
...
MDS có **10 theme chính thức** trong `assets/tokens/themes/` (blue — mặc định MISA `#245FDF`, indigo, cyan, teal, green, orange, red, pink, purple, blue-gray). Khi người dùng muốn chọn/đổi tone màu: mở `assets/theme-preview.html` cho họ xem và chọn, rồi import file theme tương ứng (kèm `base-colors.css`, `number.css`, `font.css`, `space-standard.css`). Đổi theme runtime bằng `data-mds-theme="<tên>"`. KHÔNG tự pha màu ngoài 10 theme này.

## Sinh FE trực tiếp bằng bộ MDS UI

Repo kèm bộ control Vue 3 + Tailwind viết sẵn ở `ui/components/` (**26 control**, gồm cả MIcon/DataTable/DatePicker/HeaderBar/Sidebar/Chart/ImageViewer) và **4 màn hình mẫu** ở `ui/templates/` (ListPage/FormPage/DetailPage/DashboardPage). Khi dựng màn hình: **clone template tương ứng làm khung** (root dùng `h-full`, mount trong container 100vh), **bắt buộc dùng control có sẵn, cấm viết HTML thô thay thế** — tra `assets/component-map.json`; copy file `.vue` cần dùng vào project + import tokens (xem `ui/README.md`, MChart cần `npm i echarts`); đổ mock data tiếng Việt giống thật; control chưa có thì dựng theo spec `references/` bằng token và ghi TODO.
