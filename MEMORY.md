...
- **26 control** trong `ui/components/` (Đợt 1+2+3 đã xong) + **4 màn hình mẫu** trong `ui/templates/` (ListPage, FormPage, DetailPage, DashboardPage).
- Playground Vite (`ui/playground`, port 5199) có nav 5 trang (hash routing `#list #form #detail #dashboard`), dùng để PD xem/duyệt trực tiếp trên web — mục đích thay thế Figma làm nơi review UI.
